...
void I2CSetup(void);
void performI2CTransferAndMonitor(void);
...
void GPIO_Setup(void){
    ...
    GPIO_PinModeSet (port_LED , 2, gpioModePushPullDrive, 0);                       // GPIO setup for LED0
    GPIO_DriveModeSet(port_LED , gpioDriveModeLowest);

    GPIO_PinModeSet (port_LED , 3, gpioModePushPullDrive, 0);
    GPIO_DriveModeSet(port_LED , gpioDriveModeLowest);                        // GPIO setup for LED1
...
void powerdown()
{
	...
	GPIO_PinModeSet(port_i2c_SCL, pin_i2c_SCL, gpioModeDisabled, 0);				/*GPIO pins for I2C - SDA and SCL*/
    GPIO_PinModeSet(port_i2c_SDA, pin_i2c_SDA, gpioModeDisabled, 0);				//SDA pin configured
  	GPIO_IntConfig(port_i2c_interrupt, pin_i2c_interrupt, false, true, false);			//configure external interrupt
	GPIO_PinModeSet(port_i2c_interrupt, pin_i2c_interrupt, gpioModeDisabled, 0);
    GPIO_PinModeSet(port_i2c_power, pin_i2c_power, gpioModeDisabled, 0);			// disable power to TSL2561
    GPIO_PinOutClear(port_i2c_power, pin_i2c_power);
...
void ClearTSLInterrupt(void)
{
		 	   ...
		 	   I2C1->TXDATA = TSLAddr_write;							// give address to slave for clearing interrupt on TSL
		 	   I2C1->CMD = I2C1_START;
 ...
 *****************************************************************************/
 void GPIO_ODD_IRQHandler(void)
	...
	blockSleepMode(EM1);										// enter energy mode EM1 on starting communication with the slave device
	 ...
	 I2C1->TXDATA = TSLAddr_write;								// send address of device
     I2C1->CMD = I2C1_START;									// send start signal

     while(! (I2C1->IF & I2C_IF_ACK));							// wait for ACK

     ClearACK();												// clear ACK after receipt

     writetoTXDATA( Addr_ADCval );								// access ADC registers on TSL2561 through command reg

     I2C1->CMD = I2C1_START;									// send a repeat start
     writetoTXDATA( TSLAddr_read );								// send device address with LSB = 1 for read

     while (!(I2C1->IF & I2C_IF_RXDATAV));						// start reading
     ...
     unblockSleepMode(EM1);
            ...
            a = 256*i2c_txBuffer3 + i2c_txBuffer2;						// calculate output of ADC0 and compare
                  if (a<low_threshold_light )
                  {
                	  GPIO_PinOutSet(port_LED, LED_lightness_pin);

                  }

           b = 256*i2c_txBuffer5 +  i2c_txBuffer4;						// calculate output of ADC1 and compare
                 if(b>upper_threshold_light)
                 {
                	 GPIO_PinOutClear(port_LED, LED_lightness_pin);
...
void I2CSetup(void)
{
    ...
    CMU_ClockEnable(cmuClock_GPIO, true);
	  ...
	  GPIO_PinModeSet(port_i2c_SCL, pin_i2c_SCL, gpioModeWiredAndPullUpFilter, 1);
	  GPIO_PinModeSet(port_i2c_SDA, pin_i2c_SDA, gpioModeWiredAndPullUpFilter, 1);
	...
	for (int i = 0; i < 9; i++)
	  {
	     ...
	     */
	    GPIO_PinModeSet(port_i2c_SCL, pin_i2c_SCL, gpioModeWiredAnd, 0);
	    GPIO_PinModeSet(port_i2c_SCL, pin_i2c_SCL, gpioModeWiredAnd, 1);
	  }
...
//   ClearACK();
   ...
   GPIO_PinModeSet(port_i2c_interrupt, pin_i2c_interrupt, gpioModeInput, 1);
   	GPIO_IntConfig( port_i2c_interrupt, pin_i2c_interrupt, false, true, true );
...
void LETIMER0_IRQHandler(void){
	...
	/* Wait for warmup */
	  ...
	  GPIO_PinOutSet(port_ADC_excite, 6);                // Enable the excite
	}
	...
	else if ((currentFlags & LETIMER_IF_UF) != 0)
	{
		...
		if(iflag & ACMP_STATUS_ACMPOUT)
		{
			GPIO_PinOutClear(port_LED, LED_lightness_pin);
			ACMP0->INPUTSEL &= ACMP_retain_particular_bits;
		    ACMP0->INPUTSEL |= Light_Sensor_Darkness_Ref;                     //For a high ACMP output turn off LED
			}
		else
			{GPIO_PinOutSet(port_LED, LED_lightness_pin);
			ACMP0->INPUTSEL &= ACMP_retain_particular_bits;                                    //For a high ACMP output turn on LED
		    ACMP0->INPUTSEL |= Light_Sensor_Light_Ref;
			}
			ACMP0->CTRL &= ~ACMP_CTRL_EN;                                     // Turn off ACMP
		GPIO_PinOutClear(port_ADC_excite, 6);                                       // Clear GPIO port used for excite
     }
